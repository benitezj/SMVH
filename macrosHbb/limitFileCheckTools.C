...
string m_fileNameRef;
string m_fileNameTest;
string m_region = "";
string m_subFolder ="";
...
TH1* getHisto(TDirectory* source, string histoName,  string systematic = "", string direction = "") {
  ...
  if (histNameTString.Contains("allPTV") || histNameTString.Contains("highPTV")) {
    ...
    for (unsigned int j = 0; j < regions.size(); j++) {
      ...
      if (histSum == 0) {
        histSum = getHisto(source, histNameTemp.Data(), systematic, direction);
      } else {
        histSum -> Add(getHisto(source, histNameTemp.Data(), systematic, direction));
      }
    }
    if (histSum)
...
vector<string> getListOfSystematics(TDirectory* source) {
  ...
  while ((key = (TKey*) nextkey())) {
    ...
    if (!cl) continue;
      ...
      // TODO this works for one sided systemtics if only up is present, what about only down?
      if (!m_nominalOnly 
	  && sysName.Contains("_Sys") 
...
vector<string> getListOfRegions(TDirectory* source, string sample = "data") {
  source -> cd();
  if (m_subFolder != "")
    source -> cd(m_subFolder.c_str());
...
vector<string> getListOfSamples(TDirectory* source) {
  ...
  while ((key = (TKey*) nextkey())) {
    ...
    if (cl -> InheritsFrom(TH1::Class())){
      ...
      TString regionName = key -> GetName();
      regionName.Remove(0,length+1);
      if( regionName.CompareTo(m_region.c_str()) !=0 ) continue;

      histName.Remove(length, histName.Length());
      if (histName == "EventLoop") continue;

      string histNameStr =  key -> GetName(); 
      ...
      // 	cout << "Found sample: "<< histNameStr << endl;
      // }
    }
  }
  ...
  vector<string> samplesOrdered;
  double lastMaxYield=1e9;
  for(unsigned int i=0;i<samples.size();i++){
    ...
    for(unsigned int j=0;j<samples.size();j++){
      ...
      TH1F * h = (TH1F*)gDirectory->Get(samples[j].c_str());
      if(!h){
	cout<<"something is wrong , h is NULL"<<endl;
	return samplesOrdered;
      }

      //cout<<"     "<<j<<":"<<h->GetName()<<" "<<h->Integral()<<" "<<lastMaxYield<<" "<<maxYield<<" "<<maxIdx<<endl;

      if(h->Integral()>=lastMaxYield) continue;
      
      if(h->Integral()>maxYield){
	maxYield=h->Integral();
	maxIdx=j;
      }

    }
    //cout<<"   OrderSamples : "<<maxIdx<<" "<<maxYield<<endl;    

    if(maxIdx==-1){
      cout<<"Something is wrong maxIdx=-1"<<endl;
    }
   
    
    TString histName = samples[maxIdx];
    int length = histName.First("_");
    if (length < 1) continue;
    histName.Remove(length, histName.Length());
    string histNameStr= histName.Data();
    samplesOrdered.push_back(histNameStr);
    
    cout<<"OrderedSamples : "<<maxYield<<" "<<histNameStr<<endl;
    lastMaxYield = maxYield;
  }



  cout << "Found " << samplesOrdered.size() << " samples." << endl;
  return samplesOrdered;
}
...
bool sampleAvailable(string fileName, string sample, string region) {
  TFile* file = getFile(fileName);
  string histoName = sample + "_" + region;
  TH1* h_nominal = getHisto(file, histoName);
  if (!h_nominal)
    return false;
  delete h_nominal;
  return true;
...
bool sampleZero(string fileName, string sample, string region) {
  TFile* file = getFile(fileName);
  string histoName = sample + "_" + region;
  TH1* h_nominal = getHisto(file, histoName);
  if (!h_nominal)
...
bool sampleLowStats(string fileName, string sample, string region) {
  TFile* file = getFile(fileName);
  string histoName = sample + "_" + region;
  TH1* h_nominal = getHisto(file, histoName);
  if (!h_nominal)
...
vector<Yield> readYields(string fileName, vector<string> samples, string region) {
...
vector<SysParam*> readSysFromLimitFile(string fileName, string sample, string region) {
  ...
  vector<string> systematics = getListOfSystematics(file);
  ...
  for (unsigned int i = 0; i < systematics.size(); i++) {
    ...
    TH1* h_sysUp = getHisto(file, histoName, systematics[i], sysUpName);
    TH1* h_sysDo = getHisto(file, histoName, systematics[i], sysDownName);
    TH1* h_sysUpNorm = getHisto(file, histoName, systematics[i], sysUpName);
    TH1* h_sysDoNorm = getHisto(file, histoName, systematics[i], sysDownName);
...
void checkForMissingSys(vector<SysParam*> sysParamsTest, vector<SysParam*> sysParamsRef) {
  ...
  ofstream outfile;
  outfile.open((m_plotDir+"/checkForMissingSys_"+m_region+".txt").c_str());

  outfile<<"-------------------------------------------------------- "<<endl;
  outfile<<"Missing in : "<<m_fileNameTest.c_str()<<endl;
  ...
  outfile<<"-------------------------------------------------------- "<<endl;
  outfile<<"Missing in : "<<m_fileNameRef.c_str()<<endl;
...
void printCompare(double test, double ref, ofstream o) {
  o<<setw(m_width) << formatString(test/ref);  
}

void compareYields(vector<Yield> yieldsTest, vector<Yield> yieldsRef) {
  
  ofstream outfile;
  outfile.open((m_plotDir+"/compareYields_"+m_region+".txt").c_str());

  outfile<< m_fileNameTest.c_str()<<endl;
  outfile<< m_fileNameRef.c_str()<<endl;


  outfile<< setw(m_width) << " ";
  outfile<< setw((int) (1.5 * m_width)) << m_testName;
  outfile<< setw(2 * m_width) << m_refName;
  outfile<< endl;

  outfile<< setw(m_width) << "sample";
  outfile<< setw(m_width) << "yield";
  outfile<< setw(m_width) << "unc.";
  outfile<< setw(m_width) << "yield";
  outfile<< setw(m_width) << "unc.";
  outfile<< setw(m_width) << "ratio";
  outfile<< setw(m_width) << "unc.";
  outfile<< endl;

  for (unsigned int i = 0; i < yieldsTest.size(); i++) {

    outfile<< setw(m_width) << yieldsTest[i].sample;
    outfile<< setw(m_width) << TString::Format("%.2f", yieldsTest[i].integral).Data();
    outfile<< setw(m_width) << TString::Format("%.2f", yieldsTest[i].err).Data();
    ...
    for (unsigned int j = 0; j < yieldsRef.size(); j++) {
      if (yieldsTest[i].sample == yieldsRef[j].sample) {
        ...
        outfile<< setw(m_width) << TString::Format("%.2f", yieldsRef[j].integral).Data();
        outfile<< setw(m_width) << TString::Format("%.2f", yieldsRef[j].err).Data();
        outfile<< setw(m_width) << TString::Format("%.3f", ratio).Data();
        outfile<< setw(m_width) << TString::Format("%.3f", ratioErr).Data();
        //bool flagLargeDev = getRatioFlag(ratio, m_maxYieldDev);
        //printFlag("LargeDev", flagLargeDev);
	if(fabs(ratio-1) > 0.01) outfile<< "  ! ";
	if(fabs(ratio-1)/ratioErr > 1.) outfile<< "  -->LargeDev";
        found = true;
      }
    }
    if (!found) {
  ...
  for (unsigned int j = 0; j < yieldsRef.size(); j++) {
    ...
    if (!found) {
      outfile<< setw(m_width) << yieldsRef[j].sample;
      outfile<< setw(m_width) << "-";
      outfile<< setw(m_width) << "-";
      outfile<< setw(m_width) << TString::Format("%.2f", yieldsRef[j].integral).Data();
      outfile<< setw(m_width) << TString::Format("%.2f", yieldsRef[j].err).Data();
...
void compareSys(vector<SysParam*> sysParamsTest, vector<SysParam*> sysParamsRef) {
  ofstream outfile;
  outfile.open((m_plotDir+"/compareSys_"+m_region+".txt").c_str());


  outfile << "Systematics with difference in normalization shift > " << m_maxSysNormDev * 100 << "%" << endl;

  outfile << setw(6 * m_width) << "NormUp";
  outfile << setw(2 * m_width) << "NormDo";
  outfile << endl;

  outfile << setw(4 * m_width) << "Systematic";
  outfile << setw(m_width)<<" "<< m_testName <<" / "<< m_refName;
  outfile <<" "<< m_testName <<" / " << m_refName;
  outfile << endl;
  ...
  for (unsigned int i = 0; i < sysParamsTest.size(); i++) {
    ...
    if (index >= 0) {
      ...
      outfile << setw(5* m_width) << name;
      outfile<<setw(m_width) << formatString(test.shiftSysUp)<<"/"<<formatString(ref.shiftSysUp);  
      if(ref.shiftSysUp!=0.) 
	outfile<<"="<<formatString(test.shiftSysUp/ref.shiftSysUp);  
      else  outfile<<" [] ";
      outfile<<setw(m_width) << formatString(test.shiftSysDo)<<"/"<<formatString(ref.shiftSysDo);  
      if(ref.shiftSysDo!=0.) 
	outfile<<"="<< formatString(test.shiftSysDo/ref.shiftSysDo);  
      else outfile<<" [] ";
      if (flagShiftSysUp || flagShiftSysDo) {
	outfile<<setw(m_width) <<" <<--- ";
      }
      outfile << endl;
    }
